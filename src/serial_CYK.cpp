...
int main() {
  ...
  sort(
      production1, production1 + production1_num,
      [](const Production1 &a, const Production1 &b) {
        return a.child == b.child
                   ? a.parent < b.parent
                   : a.child <
                         b.child; // 如果a相同则看变量编号，如果a不相同则a小的在前面
      });
  /*
   *
      <1>::=a
  ...
  for (int i = 0; i < production1_num;
       i++) // Here we looking at part 2 of the input.
  {
  ...
  for (int i = 0; i < string_length; i++) // 从这里开始看字符串
  {
    ...
    for (int j = begin; j < end; j++) // 遍历特化表
    {
      ...
      subTree.root =
          production1[j].parent; // give this new subtree a parent -> the marker
      subTree.num = 1;
      // first every thread doing it's job and then, we need to clever reduce
      // method.
      subTreeTable[i][i][subTreeNumTable[i][i]++] =
          subTree; // put this subtree into proper place.
    }
  }

  // put all parent together.
  sort(production2, production2 + production2_num,
       [](const Production2 &a, const Production2 &b) {
         return a.child1 == b.child1
                    ? (a.child2 == b.child2 ? a.parent < b.parent
                                            : a.child2 < b.child2)
                    : a.child1 < b.child1;
       });
  ...
  if (subTreeNumTable[0][string_length - 1] > 0) {
    ...
    for (int i = 0; i < subTreeNumTable[0][string_length - 1]; i++) {
      cout << subTreeTable[0][string_length - 1][i].root << " "
           << subTreeTable[0][string_length - 1][i].num << endl;
    }
  }
  printf("%u\n", treeNum);
  return 0;
